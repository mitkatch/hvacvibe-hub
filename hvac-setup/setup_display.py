...
def show_management_screen(ip: str, gateway_id: str = ""):
    """Show QR code for management mode (local WiFi, port 8080)."""
    try:
        import qrcode
        import pygame
        ...
        title = font_title.render("Management Mode", True, BLUE)
        screen.blit(title, (tx, ty)); ty += 28

        pygame.draw.line(screen, GRAY, (tx, ty), (WIDTH - 10, ty), 1); ty += 12

        s1 = font_body.render("Scan QR or open:", True, LGRAY)
        screen.blit(s1, (tx, ty)); ty += 22

        url_surf = font_body.render(url, True, WHITE)
        screen.blit(url_surf, (tx, ty)); ty += 28

        s2 = font_body.render("Connect your phone", True, LGRAY)
        screen.blit(s2, (tx, ty)); ty += 20
        s3 = font_body.render("to the same WiFi", True, LGRAY)
        screen.blit(s3, (tx, ty)); ty += 28

        ip_label = font_small.render(f"IP: {ip}", True, LGRAY)
        screen.blit(ip_label, (tx, ty))

        pygame.display.flip()
        log.info(f"Management screen: {url}")
...
def show_unconfigured_screen():
    """Flashing 'Hold button to setup' for first boot / no WiFi."""
    try:
        import pygame
        import threading
        ...
        def _flash():
            ...
            while not _flash_stop.is_set():
                ...
                if visible:
                    msg = font_body.render("Hold button 3s to setup", True, WHITE)
                    screen.blit(msg, (WIDTH // 2 - msg.get_width() // 2, 140))

                    sub = font_small.render("WiFi not configured", True, GRAY)
        ...
        t = threading.Thread(target=_flash, daemon=True, name="flash-screen")
        t.start()
        log.info("Unconfigured flash screen started")
        return _flash_stop   # caller can set this to stop flashing
